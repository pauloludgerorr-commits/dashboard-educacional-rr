...
dados["LOCALIZACAO"] = dados.apply(classificar, axis=1)
...
ranking = dados.groupby("NO_ENTIDADE")[["AI","AF","EM", "EJA"]].sum().reset_index()
...
graf_rank = px.bar(
    ...
    y="NO_ENTIDADE",
    orientation="h",
    title="Ranking de Todas as Escolas por Matrículas",
    height=1500
)
...
if total_em < total_af:
    problemas.append("Possível evasão na transição do Fundamental para o Ensino Médio.")
...
if loc.loc[loc.LOCALIZACAO=="Indígena","EM"].sum() == 0:
    problemas.append("Oferta limitada de Ensino Médio indígena.")
...
<h2>Distribuição por Etapa de Ensino</h2>
...
with open("dados_censo_escolar_roraima.html","w",encoding="utf-8") as f:
    f.write(html)
